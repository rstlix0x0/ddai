...
#[allow(dead_code)]
pub(crate) struct Manager<P, RP, PW>
where
    P: Processor,
    ...
    PW: PathBufWrapper,
{
    ...
    pub(crate) fn define(
        ...
        let _ = self.processor.define(definition.clone(), file_version.clone())?;
...
mod tests {
    ...
    mod test_define {
        ...
        fn test_define_new_business_def() {
            ...
            registry_processor.expect_parse().returning(move |_| Ok(expected_registry.clone()));
            ...
            let manager = Manager::new(processor, registry);
        ...
        fn test_define_existed_business_def() {
            ...
            let registry = RegistryManager::new(registry_processor, path_buf_wrapper);
            let manager = Manager::new(processor, registry);
            let result = manager.define(Definition::from("test_file"), Some(FileVersion::from("1.0.0")));
            assert!(result.is_ok())
        }
    }
}
